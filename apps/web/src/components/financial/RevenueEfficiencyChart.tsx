...
export const RevenueEfficiencyChart = memo(function RevenueEfficiencyChart({
  ...
  const options: ChartOptions<'bubble'> = {
    ...
    scales: {
      x: { border: { display: false }, grid: horizontalGrid, title: { display: true, text: 'Order Volume (Transactions) →', color: chartColor('--color-chart-label', '#475569'), font: { family: 'IBM Plex Sans, sans-serif', size: 14, weight: 800 } }, ticks: { ...chartTick(14), callback: value => formatCompact(Number(value)) } },
      y: { type: 'logarithmic', border: { display: false }, grid: horizontalGrid, title: { display: true, text: 'AOV (SEK, log scale) →', color: chartColor('--color-chart-label', '#475569'), font: { family: 'IBM Plex Sans, sans-serif', size: 14, weight: 800 } }, ticks: { ...chartTick(14), callback: value => formatCompact(Number(value)) } },
    },
  };
