...
public interface IFinancialService
{
    ...
    /// </summary>
    Task<KpiDto> GetKpisAsync(ResolvedPeriod period, TenantId? tenantId = null);

    /// <summary>
    /// Retrieves revenue velocity points for the specified timeframe.
    /// </summary>
    Task<IReadOnlyList<VelocityPointDto>> GetVelocityAsync(ResolvedPeriod period, TenantId? tenantId = null);

    /// <summary>
    /// Retrieves running accumulated revenue for current and previous periods.
    /// </summary>
    Task<IReadOnlyList<AccumulatedRevenuePointDto>> GetAccumulatedRevenueAsync(ResolvedPeriod period, TenantId? tenantId = null);
    ...
    /// </summary>
    Task<IReadOnlyList<NetGrowthAdditionPointDto>> GetNetGrowthAdditionAsync(ResolvedPeriod period, TenantId tenantId);

    /// <summary>
    /// Generates a distribution histogram of order values for a specific tenant.
    /// </summary>
    Task<IReadOnlyList<OrderBinDto>> GetOrderDistributionAsync(ResolvedPeriod period, TenantId tenantId, int? binCount = null);

    /// <summary>
    /// Analyzes transaction density by day of week and hour of day.
    /// </summary>
    Task<IReadOnlyList<TransactionDensityPointDto>> GetTransactionDensityAsync(ResolvedPeriod period, TenantId? tenantId = null);

    /// <summary>
    /// Calculates the cumulative growth delta for the specified timeframe.
    /// </summary>
    Task<IReadOnlyList<CumulativeGrowthDeltaPointDto>> GetCumulativeGrowthDeltaAsync(ResolvedPeriod period, TenantId? tenantId = null);
}
