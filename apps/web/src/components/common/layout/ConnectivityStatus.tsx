...
export function ConnectivityStatus({isOnline, isBackendOnline, variant}: ConnectivityStatusProps) {
    ...
    return (
      <span
        className={variant === 'mobile'
          ? 'flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold text-red-400 bg-red-950/40 border border-red-800/40 uppercase tracking-wider whitespace-nowrap shrink-0'
  ...
  if (!isBackendOnline) {
    return (
      <span
        className={variant === 'mobile'
          ? 'flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold text-amber-400 bg-amber-950/40 border border-amber-800/40 uppercase tracking-wider whitespace-nowrap shrink-0'
