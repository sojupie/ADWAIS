...
const mobileLinkClass = (isActive: boolean, extra = '') =>
  `px-3 py-3 rounded-lg text-base font-bold transition-colors ${extra} ${
    isActive
      ? 'bg-brand-accent/15 text-brand-accent border border-brand-accent/20'
      : 'text-white/80 hover:bg-white/5 hover:text-white'
  }`;

export function MobileNavigationMenu({pathname, financialTimeframe, fleetTimeframe, onClose}: MobileNavigationMenuProps) {
  return (
