      ...
      <div className="relative z-10 flex flex-col flex-1 min-h-0 w-full min-w-0">
        <div className="flex items-center justify-between pb-2 mb-1">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm md:text-md font-bold text-primary-container">{events.length} Events</span>
          </div>
        </div>

        <div className="flex flex-col overflow-y-auto flex-1 divide-y divide-slate-50/30 custom-scrollbar w-full min-w-0">
          {isLoading ? (
            <div className="text-sm md:text-md text-primary-container italic text-center py-2">Loading today's schedule...</div>
          ) : events.length === 0 ? (
            <div className="text-sm md:text-md text-primary-container italic text-center py-2">No events scheduled for today.</div>
          ) : (
            events.map(e => (
              <div 
                key={e.id} 
              ...
              >
                ...
                <div className="text-sm md:text-md text-primary-container truncate min-w-0 leading-normal select-text flex-shrink-0 ml-[22px]">
