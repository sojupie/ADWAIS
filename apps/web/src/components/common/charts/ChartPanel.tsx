...
export function ChartPanel({ title, subtitle, comparison, legend, bodyClassName = '', className = '', isLoading = false, isStale = false, children }: ChartPanelProps) {
    ...
    return (
        <div className={`bg-white rounded-xl border border-slate-200 shadow-sm px-4 pt-2 flex flex-col min-h-[350px] contained:min-h-0 relative overflow-hidden ${className}`}>
            {isStale && !isLoading && (
