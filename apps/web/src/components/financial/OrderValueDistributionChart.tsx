...
import type { OrderValueBucket } from '@types';
import { formatNumber } from '@utils';
import { ChartPanel } from '../common/ChartPanel';
import './OrderValueDistributionChart.css';

interface Props {
  buckets: OrderValueBucket[];
}
...
export function OrderValueDistributionChart({ buckets }: Props) {
  return (
    <ChartPanel title="Order Value Distribution" bodyClassName="order-value-distribution-chart">
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={buckets} margin={{ top: 8, right: 10, left: 8, bottom: 42 }}>
